...
public class ProgressTableCell<S> extends TableCell<S, Double> {
    ...
    public void startProgress() {
		...
		/*====================FILL CODE============================
		Alter this line to make UI still responsive while loading*/
        Thread t = new Thread(() -> {
    ...
    public void simulateProgressIncrement(Double start) {
        ...
        while (value < 1d) {
            try {
				...
				/*====================FILL CODE============================
				This is JavaFX commands. It will error if called in another thread.
				Alter this line to make it works.*/
                Platform.runLater(() -> {
    ...
    public void commitEdit(Double item) {
        if (!isEditing() && !item.equals(getItem())) {
            ...
            if (table != null) {
                ...
                CellEditEvent<S, Double> event = new CellEditEvent<>(
                        table, new TablePosition<S, Double>(table, getIndex(), column),
