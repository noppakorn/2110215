...
public class PlayTest  extends ShopApp {
	...
	Player[] opponentList = {
			new Player("Yugi Muto", 300, 55, 10),
			new Player("Sendou Aichi", 400, 35, 15),
	...
	public void run()  {
		...
		while (player.getCurrentLifePoint() > 0 && opponent.getCurrentLifePoint() > 0 ) {
			...
			if(player.getAssignedCharacter() instanceof ExodiaCharacterCard 
					&& ((ExodiaCharacterCard)(player.getAssignedCharacter())).winConditionCheck(player.getInventory()) ) {
				out.println(opponent.getName() +": Draw your last pathetic card so I can end this, " + player.getName());
				out.println(player.getName() +": My deck has no pathetic cards, "+ opponent.getName()+ ". But it does contain, the unstoppable Exodia! ");
	...
	void UseCard () {
		...
		if(card instanceof MainCharacterCard 
			...
			if(choiceC == 1) {
				...
				out.println("You switch your character to " + card.getName() + ", All of previous character items are lost");
			}
			else if(choiceC == 2) {
				int heal = ((CharacterCard) card).sacrifice(player);
				out.println("You sacrifice " + card.getName() + " card, " + player.getName() + " gains "+ heal + " Life Point");
			}
			else if(choiceC == 3) {
