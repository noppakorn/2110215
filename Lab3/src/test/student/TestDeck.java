...
public class TestDeck {
	...
	public void testRemoveCard() {
		// Try removing from middle of deck2
		try {
			assertEquals(deck2.removeCard(4), BasicCha2);
		} catch (RemoveCardFailedException e) {
			fail("Invalid Index");
		}
		Card[] cardList2AfterRemove = { BasicCha1, BasicCha1, BasicCha1, BasicCha1, BasicCha2, BasicCha2, BasicCha2 };
		assertEquals(7, deck2.getDeckSize());
		for (int i = 0; i < deck2.getDeckSize(); ++i) {
			assertEquals(cardList2AfterRemove[i], deck2.getDeckList()[i]);
		}

		// Try removing from middle of deck3
		try {
			assertEquals(deck3.removeCard(4), ItemCard1);
		} catch (RemoveCardFailedException e) {
			fail("Invalid Index");
		}
		Card[] cardList3AfterRemove = { BasicCha1, BasicCha1, BasicCha2, BasicCha2, ItemCard1, ItemCard2, ItemCard2 };
		assertEquals(7, deck3.getDeckSize());
		for (int i = 0; i < deck3.getDeckSize(); ++i) {
			assertEquals(cardList3AfterRemove[i], deck3.getDeckList()[i]);
		}
	}

	@Test
	public void testNonExistanceRemoveCard() {
		assertThrows(RemoveCardFailedException.class, () -> {
			deck1.removeCard(0);
		});
		assertThrows(RemoveCardFailedException.class, () -> {
			deck2.removeCard(100);
		});
		assertThrows(RemoveCardFailedException.class, () -> {
			deck3.removeCard(1111111111);
		});
	}
}
